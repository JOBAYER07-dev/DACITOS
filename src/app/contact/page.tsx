...
import { Phone, Mail, MessageCircle, Calendar, Send, ShieldCheck, Clock, Zap } from "lucide-react";
import Reveal from "@/components/Reveal";

const quickCards = [
  { icon: Phone, title: "Call us", desc: "Speak directly with our team", value: "+91 85408 68767" },
  { icon: Mail, title: "Email us", desc: "Send a detailed message", value: "contact@dacitos.com" },
  { icon: MessageCircle, title: "Live chat", desc: "Instant support, available 24/7", value: "Start a chat" },
  { icon: Calendar, title: "Book a meeting", desc: "Free 30-minute consultation", value: "Schedule now" },
];

const whyChoose = [
  { icon: Clock, title: "24/7 Support", desc: "Round-the-clock technical support and assistance." },
  { icon: ShieldCheck, title: "Data Security", desc: "Enterprise-grade security across every project." },
  { icon: Zap, title: "Fast Response", desc: "Quick turnaround and efficient delivery, every time." },
];
          ...
          {quickCards.map((c, i) => (
            <Reveal key={c.title} delay={i * 0.07}>
              <div className="glass-card rounded-2xl p-6 h-full">
                ...
                <h3 className="font-display font-medium text-text-primary">{c.title}</h3>
                <p className="mt-1.5 text-xs text-text-muted">{c.desc}</p>
                <p className="mt-3 text-sm text-cyan-soft font-medium">{c.value}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-[1.2fr_0.8fr] gap-10">
          <Reveal>
            <div className="glass-card rounded-3xl p-8 sm:p-10">
              <h2 className="font-display text-2xl font-semibold text-text-primary">Send us a message</h2>
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                ...
                <div>
                  <label className="text-xs font-mono text-text-muted">Project details *</label>
                  <textarea
                    required
                    rows={5}
                    value={form.message}
                ...
                {status === "success" && (
                  <p className="text-sm text-cyan-soft text-center">
                  ...
                  {whyChoose.map((w) => (
                    <div key={w.title} className="flex items-start gap-3.5">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-text-muted mt-0.5">{w.desc}</div>
                      </div>
                    </div>
                  ))}
                </div>
